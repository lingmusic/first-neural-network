...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        # as shown below.
        self.activation_function_sigmoid = lambda x : 1 / (1 + np.exp(-x))  # Replace 0 with your sigmoid calculation.
        self.activation_function_sigmoid_prime = lambda x : self.activation_function_sigmoid(x) * (1 - self.activation_function_sigmoid(x))
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_outputs = self.activation_function_sigmoid(hidden_inputs) # signals from hidden layer
            ...
            output_error_term = error * self.activation_function_prime(final_outputs)
            
            # TODO: Calculate the hidden layer's contribution to the error
            hidden_error = np.dot(self.weights_hidden_to_output,output_error_term) 
            
            # TODO: Backpropagated error terms - Replace these values with your calculations.
            hidden_error_term =  hidden_error * self.activation_function_sigmoid_prime(hidden_inputs)
...
class TestMethods(unittest.TestCase):
    ...
    def test_activation(self):
        ...
        self.assertTrue(np.all(network.activation_function_sigmoid(0.5) == 1/(1+np.exp(-0.5))))
...
# 
# #### 请将你的答案填写在下方
# 预测结果在圣诞节前后这段特殊的假期时间段不够准确，这是因为数据模型只有一年的数据量，且这期间的数据没有放入训练数据，所以在这期间的预测准确率较低是可以解释的。
